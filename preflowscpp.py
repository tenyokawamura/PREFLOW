...
def preflowscpp(engs, params, fluxes):
    # ------------------------------------- #
    # ---------- Initial setting ---------- #
    # ------------------------------------- #
    inpar=SetParameter()
    inpar.preflowscpp_set_inpar(pars=params, es=engs)
    ...
    if inpar.quant==0:
        ...
        pars=[\
            inpar.gamma_d, inpar.temp_bb_d, inpar.temp_e_d, inpar.norm_d,\
        ...
        specs=enespec.preflowscpp_calc_spectra(es=inpar.es, pars=pars)
    ...
    #########################
        ...
        if [inpar.cf_sr, inpar.cf_hr]==[0., 0.]:
            # Subject band
            rings.ws=preflowscpp_weight_calc(\
            ...
            # Reference band
            rings.ws_r=preflowscpp_weight_calc(\
        ...
        # --- Reverberation taken into account --- #
            ...
            # Subject band
            rings.ws, rings.wrss=preflowscpp_ref_weight_calc(\
                e_min=inpar.e_min,       e_max=inpar.e_max,     pars_spec=pars_spec,\
                eta0_d=inpar.eta0_d,     eta1_d=inpar.eta1_d,\
                eta0_s=inpar.eta0_s,     eta1_s=inpar.eta1_s,\
                eta0_h=inpar.eta0_h,     eta1_h=inpar.eta1_h,\
                ...
                fs=tifr.fs)
            # Reference band
            rings.ws_r, rings.wrss_r=preflowscpp_ref_weight_calc(\
                e_min=inpar.e_minr,      e_max=inpar.e_maxr,    pars_spec=pars_spec,\
                eta0_d=inpar.eta0_d,     eta1_d=inpar.eta1_d,\
                eta0_s=inpar.eta0_s,     eta1_s=inpar.eta1_s,\
                eta0_h=inpar.eta0_h,     eta1_h=inpar.eta1_h,\
